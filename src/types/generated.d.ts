...
type EnterpriseUserAccount = Node & Actor & {
...
type IpAllowListOwner = Enterprise | Organization;
...
type Organization = Node & Actor & PackageOwner & ProjectOwner & RepositoryOwner & UniformResourceLocatable & MemberStatusable & ProfileOwner & Sponsorable & {
...
type SearchResultItem = App | Issue | MarketplaceListing | Organization | PullRequest | Repository | User;
...
type User = Node & Actor & PackageOwner & ProjectOwner & RepositoryOwner & UniformResourceLocatable & ProfileOwner & Sponsorable & {
  /** Determine if this repository owner has any items that can be pinned to their profile. */
  anyPinnableItems: Scalars['Boolean'];
...
type PinnedRepositoriesQuery = { user?: Maybe<{ pinnedItems: { nodes?: Maybe<Array<Maybe<Pick<Repository, 'id' | 'name' | 'descriptionHTML' | 'url' | 'homepageUrl'>>>> } }> };
...
type ProfileInfoQuery = { user?: Maybe<Pick<User, 'name' | 'email' | 'bioHTML'>> };
